...
//248
//3.2k1.4k
// Code Issues 4 Pull requests 38 Actions Projects 0 Wiki Security Insights
//java-algorithms-implementation/src/com/jwetherell/algorithms/data_structures/BTree.java
//@ParkChangHan ParkChangHan Fixed Param of Comment
//d428afe on 20 Nov 2017
//@phishman3579@ParkChangHan
//898 lines (804 sloc)  28.6 KB
...
public class BTree<T extends Comparable<T>> //implements ITree<T>
{
    ...
    public BTree(String tableName, String strColName, int nodeSize,String strColType ) {
    	this.minKeySize = (int) Math.ceil((maxKeySize+1)/2)-1;
        this.minChildrenSize = minKeySize + 1;
        this.maxKeySize = nodeSize ;
        this.maxChildrenSize = maxKeySize + 1;
        this.columnName = strColName;
    }
...
//    @Override
    public boolean add(T value) {
        if (root == null) {
            ...
            Node<T> node = root;
            while (node != null) {
                if (node.numberOfChildren() == 0) { //reached a leaf
                    node.addKey(value);
                ...
                // Navigate             not leaf(searching for node)
    ...
    /**
     * find page for insertion
     * 
     * 
     */
    public ArrayList<String> getInsertPage(T value) {
        if (root == null) {
            root = new Node<T>(null, maxKeySize, maxChildrenSize);
            root.addKey(value);
        } else {
...
//                    // Need to split up
//                    split(node);
//                    break;              //no need to do splitting as no insertion anyway
                }
                // Navigate             not leaf(searching for node)
...
//    @Override
    ...
    private static class Node<T extends Comparable<T>> {
        ...
        private Node(Node<T> parent, int maxKeySize, int maxChildrenSize) {
            this.parent = parent;
            this.keys = (T[]) new Pointer[maxKeySize + 1];
            this.keysSize = 0;
        ...
        private ArrayList<String> getNodePages(){
        	ArrayList<String>pointers  = new ArrayList<String>();
        	for(T pointer:keys) {
        		String tempPage = ((Pointer)pointer).getPage();
        		pointers.add(tempPage);
        	}
        	return pointers;
        }
